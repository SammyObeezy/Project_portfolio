...
export function getCombinedChildText(node) {
    ...
    return React.Children.toArray(node.children)
        .filter((child) => _.has(child, 'props')) // filter out primitive types
        .map((child) => getCombinedChildText(child.props))
        .reduce((combinedText, childText) => combinedText + childText, _.find(React.Children.toArray(node.children), _.isString) || '');
}
/**
 ...
 * @return boolean
 */
export function propsSearch(text = null, node = null) {
    if (!text) {
        return true;
    }
