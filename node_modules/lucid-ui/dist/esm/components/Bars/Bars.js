...
export class Bars extends PureComponent {
    ...
    render() {
        ...
        const innerXScale = d3Scale
            .scaleBand()
            .domain(_.times(yFields.length, (num) => `${num}`))
        ...
        return (React.createElement("g", { ..._.omit(passThroughs, nonPassThroughs), className: cx(className, '&') }, _.map(transformedData, (series, seriesIndex) => (React.createElement("g", { key: seriesIndex },
            ...
            React.createElement(PureToolTip, { isExpanded: hasToolTips && hoveringSeriesIndex === seriesIndex, height: isStacked
                    ...
                    : // prettier-ignore
                        //@ts-ignore
                        yScale.range()[0] - yScale(_.max(_.flatten(series))), width: xScale.bandwidth(), 
                // @ts-ignore
                x: xScale(data[seriesIndex][xField]), y: yScale(_.max(_.flatten(series))), series: series, seriesIndex: seriesIndex, onMouseEnter: this.handleMouseEnter, onMouseOut: this.handleMouseOut, xFormatter: xFormatter, xField: xField, renderBody: renderTooltipBody || this.defaultTooltipFormatter, data: data }))))));
    }
}
