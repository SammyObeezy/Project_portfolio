...
export const Legend = (props) => {
    ...
    return (React.createElement("ul", { ...passThroughs, className: cx('&', {
            '&-is-horizontal': isHorizontal,
            '&-is-vertical': isVertical,
            '&-has-borders': hasBorders,
            '&-is-reversed': isReversed,
        }, className) }, _.map(itemProps, ({ hasLine, hasPoint, pointKind = 1, color, children, className: itemClass, }, index) => (React.createElement("li", { key: index, className: cx(itemClass, '&-Item'), onClick: _.partial(handleItemClick, index, itemProps[index]) },
        hasPoint || hasLine ? (React.createElement("svg", { className: cx('&-Item-indicator'), width: hasLine || hasSomeLines ? LINE_WIDTH : POINT_SIZE, height: POINT_SIZE },
            hasPoint ? (React.createElement(Point, { x: hasLine || hasSomeLines ? LINE_WIDTH / 2 : POINT_SIZE / 2, y: POINT_SIZE / 2, color: color, kind: pointKind })) : null,
            hasLine ? (React.createElement(Line, { d: `M0,${POINT_SIZE / 2} L${LINE_WIDTH},${POINT_SIZE / 2}`, color: color })) : null)) : null,
        children)))));
};
