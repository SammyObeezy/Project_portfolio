...
const PieChart = (props) => {
    const { style, className, height, width, margin: marginOriginal, data, hasToolTips, hasStroke, palette, colorMap, isDonut, donutWidth, ToolTip: toolTipProps, isHovering, hoveringIndex, xAxisField, xAxisFormatter, yAxisField, yAxisFormatter, ...passThroughs } = props;
    const margin = {
        ...PieChart.MARGIN,
        ...marginOriginal,
    };
    ...
    if (_.isEmpty(data) || width < 1 || height < 1) {
        return (React.createElement("svg", { ...pieChartProps, style: style, className: svgClasses, width: width, height: height }));
    }
    ...
    return (React.createElement("svg", { ...pieChartProps, style: style, className: svgClasses, width: width, height: height },
        React.createElement(ToolTip, { ...toolTipProps, isLight: true, isExpanded: hasToolTips && isHovering, onMouseOver: _.noop, onMouseOut: handleMouseOut },
            React.createElement(ToolTip.Target, { elementType: 'g' },
                React.createElement("g", { transform: `translate(${margin.left}, ${margin.top})` },
                    React.createElement("g", { transform: `translate(${innerWidth / 2}, ${innerHeight / 2})` }, _.map(pieData, (pieDatum, index) => {
                        ...
                        return (React.createElement("g", { key: index, className: cx('&-slice-group', {
                                '&-slice-group-is-hovering': isHovering && hoveringIndex === index,
                            }) },
                            React.createElement(Line, { key: index, className: cx('&-slice', {
                                    '&-slice-has-stroke': hasStroke,
                                }), d: arcData, color: _.get(colorMap, (data && data[index][xAxisField]) || '', palette[index % palette.length]), transform: `scale(${isHovering && hoveringIndex === index ? HOVER_SCALE : 1})` }),
                            React.createElement("path", { className: cx('&-slice-hover'), d: arcFullData, transform: `scale(${HOVER_SCALE})`, onMouseOver: _.partial(handleMouseOver, index), onMouseOut: hasToolTips ? _.noop : handleMouseOut })));
                    })))),
            React.createElement(ToolTip.Title, null, xAxisFormatter(_.get(data, `[${hoveringIndex}].${xAxisField}`))),
            React.createElement(ToolTip.Body, null, yAxisFormatter(_.get(data, `[${hoveringIndex}].${yAxisField}`))))));
};
...
export default buildModernHybridComponent(PieChart, { reducers });
